...
#+end_src
*** Electric
This built-in package includes =electric-indent-mode= which automatically indents code on a newline and =electric-pair-mode= and which automatically insert closing delimiters.
#+begin_src emacs-lisp
...
My anime and manga notes have =progress= proprty which is =number-of-chapter-i-read/total-amount-of-chapters= with the total amount being =?= if the work didn't end.
