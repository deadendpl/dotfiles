...
Org roam can provide links to the nodes at point when =org-roam-completion-everywhere= variable is set to t.
...
The issue with node candidates is that they don't have an icon while dabbrevs have.
I fix that.
#+begin_src emacs-lisp
(use-package org-roam
