...
(use-package dired
  ...
  (defun dired-do-du (files)
    ...
    (let ((command-output
           (if (length> files 1)
               (shell-command-to-string (format "du -c -h -s -L %s"
                                                (mapconcat
                                                 #'shell-quote-argument
                                                 files " ")))
             (shell-command-to-string (format "du -h -s -L %s"
                                              (shell-quote-argument
                                               (car files)))))))
...
123/124 or 45/? for example."
  ...
  (defun org-roam-increment-progress-property (node)
    "Increment value of progress property of anime or manga NODE."
    (interactive (list
                  (org-roam-node-read
                   nil
                   (lambda (node)
  ...
  (defun org-roam-open-animanga-node (type id)
    ...
    (if-let* ((root-type (org-roam-anilist-root-format type))
              ...
              (matched-result
               (seq-find
                (lambda (item)
                  (when-let* ((props (nth 1 item))
                              (url (alist-get "ANILIST"
                                              props nil nil
                                              #'equal)))
                    (and (string-search root-type url)
                         (string-search
