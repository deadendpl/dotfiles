...
notification() {
  choice=$(notify-send --action 'action1=Open screenshot' "Screenshot of screen" "$filename")
...
choice=$(printf "Full Screen\nSelect Area\nColor Picker" | rofi -config "${XDG_CONFIG_HOME}/rofi/config-bare.rasi" -dmenu -i -l 3 -p "Screenshot Type:")
