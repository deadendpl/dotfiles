...
- [[#essential-software-to-load][ESSENTIAL SOFTWARE TO LOAD]]
...
** Zooming In/Out
You can use the bindings CTRL plus + and =/- for zooming in/out. You can also use CTRL plus the mouse wheel for zooming in/out.
...
* RELOAD EMACS
This is just an example of how to create a simple function in Emacs. Use this function to reload Emacs after adding changes to the config. I'm loading the user-init-file twice in this function, which is a hack because for some reason, just loading the user-init-file once does not work properly.
...
* THEME
The first line adds the directory for themes if you would want to add custom theme.
