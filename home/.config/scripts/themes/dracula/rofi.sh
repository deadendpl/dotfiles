...
if [ -n "$old_line" ]; then
    ...
    new_line='@theme "~/.config/rofi/themes/drac-list.rasi"'
...
if [ -n "$old_line" ]; then
    ...
    new_line='chosen=$(echo -e "󰍃 Logout\n Shutdown\n Reboot\n󰤄 Suspend" | rofi -config ~/.config/rofi/config-bare.rasi -theme ~/.config/rofi/themes/drac-small.rasi -theme-str '\''window {width: 12%;}'\'' -dmenu -l 4 -i -p Power)'
    tmp_file=$(mktemp)
